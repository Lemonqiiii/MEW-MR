...
### 11. 质量关卡设计假设与执行脱节
**问题**: Gate 1-3 设计时假设了双Agent协作条件（Cohen's Kappa需两个独立评分者、预定义金标准论文列表），但这些条件在单Agent叙述性综述流程中不具备可行性。此外，Gate 1-3 没有可执行脚本（只有文字描述），而 Gate 4/5 有可执行脚本。结果：Gate 4/5 被实际执行，但 Gate 1-3 被标记为"未执行"。
**实际运行 Gate 1-3 后的发现**:
- Gate 1.4 喉鳞癌污染检查发现了残留的 PMID 42111396(喉鳞癌)和 41669261(非鳞NSCLC)——虽然从稿件中已移除，但数据文件未清理（现已修复）
- Gate 3.2 识别出 5/18 交叉主题仅1篇文献支撑——这些是 niche/emerging topics，在叙述性综述中可接受但需标注
- Gate 3.1 笔记质量 10/10 满分——验证了论文分析Agent的工作质量
**根因**: (a) Gate 设计没有区分"系统综述级"和"叙述性综述级"要求；(b) 没有可执行脚本 = 不会被运行；(c) 关卡检查与项目执行并行而非串行——在同一天内完成检索→筛选→阅读→写作，关卡被跳过。
**规则**:
- 所有 Gate 必须有可执行脚本（PYTHON或BASH），不能只有文字描述
- Gate 设计需区分两个等级：系统综述级（严格定量指标）和叙述性综述级（实用检查指标）
- 即使项目快速推进，Phase 切换时必须提示运行对应 Gate 脚本
- 数据清理不仅要在稿件层面进行（移除引用），还要在数据文件层面进行（清理JSON）——Gate 1.4 证明了"双重清理"的必要性
- 预印本缺少 PMID 是合理现象，检查阈值需考虑此场景
