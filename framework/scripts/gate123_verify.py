#!/usr/bin/env python3
"""
[DEPRECATED] Use verify_gates.py instead:
    python3 scripts/verify_gates.py --gate 1 --gate 2 --gate 3

This script is kept for reference. It was the original Gates 1-3 checker before
the unified verify_gates.py was created in Phase 3 of the framework refactoring.
"""
...
DATA_DIR = str(ROOT / _config["paths"]["data_dir"])
PAPERS_DIR = str(ROOT / _config["paths"]["papers_dir"])
KEY_FINDINGS = str(ROOT / "state.json")  # replaced memory/key-findings.md with state.json
...
# Check 4: Domain contamination check (configurable — skipped if not configured)
print("\n[1.4] Domain contamination check")
contamination_config = _config.get("quality", {}).get("contamination_check", {})
if not contamination_config.get("enabled", False):
    print("  ⚠️  SKIP: contamination check not configured (set quality.contamination_check in config.yaml)")
else:
    contamination_keywords = contamination_config.get("keywords", [])
    lung_context_keywords = contamination_config.get("lung_context_keywords", ["lung", "pulmonary"])
    contaminated = []
    for p in screening_40:
        ...
        for kw in contamination_keywords:
            if kw.lower() in text:
                ...
                break
    if not contaminated:
        ok(f"no contamination keywords detected in {len(screening_40)} papers")
    else:
        for pm, kw, t in contaminated:
            print(f"  ⚠️  Found '{kw}' in PMID:{pm} — {t}")
            has_lung = any(lk in (p.get("title","")+p.get("abstractText","")).lower()
                           for p in screening_40 if p.get("pmid")==pm for lk in lung_context_keywords)
            if has_lung:
                print(f"     → Has lung context keyword — likely OK (e.g. comparison group)")
            else:
                fail(f"PMID:{pm} may be off-topic: '{kw}' found, no context keyword")
...
print("\n[3.3] 笔记-大纲映射")
outline_path = str(ROOT / "manuscript" / "outline.md")
